...
class MeetingDetailScreenState extends State<MeetingDetailScreen> {
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          IconButton(
              ...
              onPressed: () {
                _deleteMeeting(context, _meetingDetail.meetingId);
              })
        ],
      ),
      floatingActionButton: FloatingActionButton(
          onPressed: () {
            _pushMeetingToTop(context);
          },
          child: Padding(
              padding: EdgeInsets.only(left: _topLeftPadding),
              child: Text(_top))),
  ...
  Widget meetingDetailScreenBody(BuildContext context) {
    ...
    if (null == _bannerUrls || _bannerUrls.length <= 0)
      ...
      List<Widget> banners = [];
      _bannerUrls.forEach((url) {
        banners.add(CachedNetworkImage(imageUrl: url));
      });
              ...
              children: <TableRow>[
                TableRow(children: <Widget>[
                  Padding(
                      ...
                      child: Text("id: ${_meetingDetail.id.toString()}")),
                  Padding(
                      padding: EdgeInsets.fromLTRB(5.0, 5.0, 10.0, 5.0),
                      child: Text("城市: ${_meetingDetail.city}")),
                ]),
                TableRow(children: <Widget>[
                  Padding(
                      padding: EdgeInsets.fromLTRB(10.0, 5.0, 5.0, 5.0),
                      child: Text(
                          "日期: ${DateUtil.getFormattedTime(_meetingDetail.date)}")),
                  Padding(
                      padding: EdgeInsets.fromLTRB(5.0, 5.0, 10.0, 5.0),
                      child: Text("年龄: ${_meetingDetail.age.toString()}")),
                ]),
                TableRow(children: <Widget>[
                  Padding(
                      padding: EdgeInsets.fromLTRB(10.0, 5.0, 5.0, 5.0),
                      child: Text("身高: ${_meetingDetail.height.toString()}")),
                  Padding(
                      padding: EdgeInsets.fromLTRB(5.0, 5.0, 10.0, 5.0),
                      child: Text("描述: ${_meetingDetail.content}")),
                ]),
              ],
            ),
            Container(
  ...
  _loadMeetingDetail() {
    DioUtil.getMeetingDetail((MeetingDetail detail) {
      setState(() {
        _meetingDetail = detail;
        _bannerUrls = List<String>();
        for (int i = 0; i < _meetingDetail.pics; i++) {
          _bannerUrls.add(DioUtil.PIC_SERVER +
              'meeting/${_meetingDetail.meetingId}/$i.jpg');
        }
      });
    }, _meetingId);
  }
  ...
  void _deleteMeeting(BuildContext context, int meetingId) {
    showDialog(
        ...
        builder: (_) => CupertinoAlertDialog(
              content: Text('你确定删除这个邀约吗?', textScaleFactor: 1.0),
              actions: <Widget>[
                GestureDetector(
                  ...
                  onTap: () {
                    setState(() {
                      ...
                      showDialog(
                          context: context,
                          builder: (_) => UnderMoonDialog('正在删除...'));
                  ...
                  child: Container(
                    ...
                    child:
                        Text('确定', style: Theme.of(context).textTheme.display1),
                  ),
                ),
                GestureDetector(
                  behavior: HitTestBehavior.translucent,
                  ...
                  child: Container(
                    ...
                    alignment: Alignment.center,
                    child:
                        Text('取消', style: Theme.of(context).textTheme.display2),
                  ),
                )
              ],
            ));
  ...
  void _pushMeetingToTop(BuildContext context) {
    showDialog(
        context: context,
        builder: (_) => CupertinoAlertDialog(
              content: Text('你确定执行这个动作吗?', textScaleFactor: 1.0),
              actions: <Widget>[
                GestureDetector(
                  behavior: HitTestBehavior.translucent,
                  onTap: () {
                    setState(() {
                      ...
                      showDialog(
                          context: context,
                          builder: (_) => UnderMoonDialog('正在执行...'));
                      DioUtil.topMeeting(
                              _meetingDetail.meetingId, _meetingDetail.top)
                          .then((result) {
                        Navigator.pop(context);
                        if (result == 1) {
                          Navigator.pop(context);
